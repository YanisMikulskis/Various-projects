...
snake = SNAKE(x_snake, y_snake, 'HEAD.png', snake_G, True, 90)
...
def make_snake(x_snake_arg, y_snake_arg, SG_ARG, angle):
    ...
    snake_s = SNAKE(x_snake_arg, y_snake_arg, rnd.choice(segments_png[:-1]), snake_G, False, angle)
    if SG_ARG:
        make_tail(SG_ARG, angle)
    # В функции создания нового сегмента будем вызывать функцию создания хвоста (она ниже).
    # Причем, два раза условие if SG_ARG мы пишем для того, чтобы создался второй сегмент, так как в нем мы
    # передаем значение None в этот параметр
    # если у нас все будет в одном условии то змея у нас будет иметь изначально только хвост и голову. а нам нужен элемент
    # как создается хвост: при столкновении с яблоком вызывается эта функция. выбирается по условию угол, затем уничтожается
    # хвост.на его место встает сегмент (snake_s) а потом вставляется хвост. используется параметр, который является списком из группы
...
while runGame:
    ...
    snake_parts_l_x = [i.rect.centerx for i in
                       snake_parts_l]  # список всех координат "х" у списка(т.е) группы частей змеи
    snake_parts_l_y = [i.rect.centery for i in
                       snake_parts_l]  # список всех координат "у" у списка(т.е) группы частей змеи
    # ------------------------------------------
    ...
    match motion:
        case 'UP':
            snake.update(snake_parts_l, 'up', SCALE_NUM)
        case 'LEFT':
            snake.update(snake_parts_l, 'left', SCALE_NUM)
        case 'DOWN':
            snake.update(snake_parts_l, 'down', SCALE_NUM)
        case 'RIGHT':
            snake.update(snake_parts_l, 'right', SCALE_NUM)
    # ------------------------------------------
    if apple_body.rect.colliderect(snake_parts_l[0].rect):  # столкновение с яблоком
        ...
        apple_body = APPLE(make_x(), make_y(), 'OB_SNAKE.png', [5, 5], None,
                           True)  # удаление тела яблока и создание нового
        # ------------------------------------------
