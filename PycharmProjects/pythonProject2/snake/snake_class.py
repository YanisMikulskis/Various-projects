...
class SNAKE(pg.sprite.Sprite):
    def __init__(self, x, y, filename, group, head_snake, angle):
        ...
        # ------------------------------------------
        if head_snake:
            self.arr, self.map_rotates = [], []  # список-след движения головы(коллекция координат)
    ...
    def update(self, snake_segments, vector, scale_arg, next_cords=0):
        ...
        match vector:
            ...
            case 'up':
                ...
                if self.VARROTHEAD != 'up':
                    self.VARROTHEAD = 'up'
                    if self.rect.centerx < snake_segments[1].rect.centerx:
                        Foo_ROTATE(-90)
                    elif self.rect.centerx > snake_segments[1].rect.centerx:
                        Foo_ROTATE(-270)
                    elif self.rect.centery > snake_segments[1].rect.centery:
                        Foo_ROTATE(180)
            case 'left':
                self.rect.centerx -= self.speed
                if self.VARROTHEAD != 'left':
                    self.VARROTHEAD = 'left'
                    if self.rect.centery < snake_segments[1].rect.centery:
                        Foo_ROTATE(90)
                    elif self.rect.centery > snake_segments[1].rect.centery:
                        Foo_ROTATE(-90)
                    elif self.rect.centerx > snake_segments[1].rect.centerx:
        ...
        # ------------------------------------------
        self.arr.append(
            list(snake_segments[0].rect.center))  # список координат (след), который прошел первый элемент (голова)
            ...
            if self.freedrive:  # начало движения по следу. код выше для первого после головы элемента уже не нужен
                ...
                if len(snake_segments[1:]) >= 4:
                    if len(self.arr[:-self.image.get_width() * len(snake_segments[1:])]) >= self.image.get_width() * 2:
                        del self.arr[0]
                    if len(self.map_rotates[
                           :-self.image.get_width() * len(snake_segments[1:])]) >= self.image.get_width() * 2:
