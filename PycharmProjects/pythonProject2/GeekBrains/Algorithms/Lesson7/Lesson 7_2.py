...
def deco(func):
    def wrapper(*args):
        t1, m1 = default_timer(), memory_usage()[0]
        result = func(*args)
        t2, m2 = default_timer(), memory_usage()[0]
        print(f'Память : {m2 - m1} MiB\nВремя: {(t2 - t1).__round__(3)}')
        return result
    return wrapper
...
def gnome_sort(m: int):
    ...
    arr_sort = list(sorted(arr))
    print(f'Массив отсортирован!\n{arr}') if arr == arr_sort else ...
...
def shell_sort(m: int):
    ...
    #сортировка шелла быстрее гномей сортировки и лучше работает по памяти
#
shell_sort(1000)

# lst_range = [2, 8, 7, 3, 9, 10, 3, 5, 0, 6, 4]
# def shell_sort_2(lst):
#     N = len(lst)
#     dist = N // 2
#     ranges = dist
#     iterations = 0
#     border = 1 if not N % 2 else 2
#     print(f'дист {dist}')
#     while N > 1:
#         N = N // 2
#         iterations += 1
...
#         if i > 1:
#             border = 1
#         for item in range(ranges * i + border):
#             print(lst)
#             print(f'сравнение {lst[item]}     {lst[item + dist]}')
#             if i == 1 and item == dist + 1:
#                 item = 0
#
#             try:
#                 if lst[item] > lst[item + dist]:
#                     lst[item], lst[item + dist] = lst[item + dist], lst[item]
#             except IndexError:
#                 break
#         dist //= 2
#
